...
exports.postExpense = async (req, res) => {
  ...
  try {
    ...
    const response = await req.user.createExpense(
      { day: day, month: month, year: year, amount, description, category },
      { transaction: t }
    );
    ...
    // });
    await t.commit();
    res.status(200).json({ expense: response, user: totalExpenses });
  } catch (e) {
    await t.rollback();
...
exports.deleteExpense = async (req, res) => {
  ...
  try {
    ...
    const currentExpense = +req.user.totalExpense - amount;
    // console.log("first",req.user.totalExpense);
    await User.update(
      {
    ...
    const noofrows = await Expense.destroy(
      ...
      { transaction: t }
    ); //authenticate user only allowed to delete
    if (noofrows === 0) {
      return res
        .status(404)
    ...
    res
      .status(200)
      .json({ success: true, user: currentExpense, msg: "Deleted Successful" });
  } catch (e) {
    await t.rollback();
...
exports.getExpenses = async (req, res) => {
  try {
    ...
    //   attributes: ["totalExpense"],
    // });
    const user = req.user.totalExpense;
    res.status(200).json({
      success: true,
      products: expenses,
      user: user,
